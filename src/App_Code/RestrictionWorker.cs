...
public class RestrictionWorker
{
    ...
    private IList<RestrictionResult> AddInfos(IdentifyResult[] irs)
    {
        ...
        foreach (IdentifyResult ir in irs)
        {
            ...
            MapServerLegendInfo legInfo = this.legendInfo.GetLegendInfo(ir.layerId);
            LayerInfoJson.Renderer renderer = this.layerInfo.GetLayerInfoAsJson(ir.layerId).drawingInfo.renderer;
            ...
            switch (renderer.type)
            {
                case "simple":
                    ...
                    break;
                case "uniqueValue":
                    string alias = this.layerInfo.GetFieldAlias(layInfo, renderer.field1);
                    string value = ir.attributes[alias];
            ...
            result.Legend = new RestrictionLegend();
            if (legendClass != null)
            {
                result.Legend = new RestrictionLegend
                {
                    Text = label,
                    TypeCode = string.Format("{0}:{1}", ir.layerId, Helper.ToHexString(label)),
                    Symbol = legendClass.SymbolImage.ImageData
                };
            }
            else
            {
                throw new WsUserException(string.Format(Resources.Resource.LEGEND_NOT_FOUND, label, ir.layerName));
            }
        ...
        foreach (int id in ids)
        {
            ...
            foreach (RestrictionResult intersect in intersects.Where<RestrictionResult>(ir => ir.LayerId == id))
            {
                ...
                if (first)
                {
                    ...
                    intersect.OtherLegends = otherLegends.ToArray();
